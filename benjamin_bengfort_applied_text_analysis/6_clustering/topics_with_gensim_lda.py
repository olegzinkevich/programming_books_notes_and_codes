...
# работает но выдает topics with POS
...
if __name__ == '__main__':
    ...
    docs = [list(corpus.docs(fileids=fileid))[0] for fileid in corpus.fileids()]
    gensim_lda.fit(docs)

#     In order to inspect the topics, we can retrieve them from the LDA step, which is the
# gensim_model attribute from the last step of our pipeline. We can then use the Gensim
# LDAModel show_topics method to view the topics and the token-weights for the
# top ten most influential tokens

    lda = gensim_lda.model.named_steps['model'].gensim_model
    lda.show_topics()

#     # We can also define a function get_topics, which given the fitted LDAModel and vectorized corpus, will retrieve the highest-weighted topic for each of the documents in the corpus get the highest weighted topic for each of the documents in the corpus
    def get_topics(vectorized_corpus, model):
        from operator import itemgetter

        topics = [
            max(model[doc], key=itemgetter(1))[0] for doc in vectorized_corpus
        ]
    ...
    corpus = [
        gensim_lda.model.named_steps['vect'].lexicon.doc2bow(doc) for doc in gensim_lda.model.named_steps['norm'].transform(docs)]
